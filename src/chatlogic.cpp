...
ChatLogic::ChatLogic()
{
    ...
    // create instance of chatbot - chabot should not be owned by chatlogic anymore
...
ChatLogic::~ChatLogic()
{
    ...
    //delete _chatBot; // no need to delete chatbot since its destructor will handle it?

    // delete all nodes - no need to delete since we use smart pointer
    // for (auto it = std::begin(_nodes); it != std::end(_nodes); ++it)
    // {
    //     delete *it;
    // }

    // delete all edges - no need to delete since should be handles by smart pointers
...
void ChatLogic::LoadAnswerGraphFromFile(std::string filename)
{
            ...
            if (type != tokens.end())
            {
                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "NODE")
                    {
                        ...
                        // make sure ownership of node is not transferred
                    ...
                    if (type->second == "EDGE")
                    {
                        ...
                        if (parentToken != tokens.end() && childToken != tokens.end())
                        {
                            ...
                            // Pass raw pointers only to the functions using get()
                            ...
                            // again move semantics to move the edge from chatlogic to graphnode
    ...
    for (auto it = std::begin(_nodes); it != std::end(_nodes); ++it)
    {
        ...
        if ((*it)->GetNumberOfParents() == 0)
        {
            ...
            if (rootNode == nullptr)
            {
                // Pass raw pointers only using get()
    ...
    rootNode->MoveChatbotHere(std::move(chatbot)); // Move chatbot ownerships from chatlogic to rootnode
